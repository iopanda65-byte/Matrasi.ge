    ...
    <header className="sticky top-0 z-50 w-full border-b border-slate-100 bg-white/80 backdrop-blur-md">
      <div className="mx-auto flex max-w-7xl h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2">
          <Link
            to="/"
            className="text-xl font-bold tracking-tight text-slate-800 hover:opacity-90 transition"
          >
            <span className="text-blue-500">☁️</span> 𝑮𝒆𝒓𝒎𝒂𝒏𝒖𝒍𝒊 𝑴𝒂𝒕𝒓𝒂𝒔𝒆𝒃𝒊
          </Link>
        </div>
      ...
      <div className="md:hidden border-t border-slate-100 bg-white px-4 py-2">
        <nav className="flex justify-around text-xs font-medium text-slate-600">
          <Link
            to="/"
            className={`flex flex-col items-center gap-1 ${isActive("/") ? "text-blue-600" : ""}`}
          >
            <span>🏠</span> მთავარი
          </Link>
          <Link
            to="/products"
            className={`flex flex-col items-center gap-1 ${isActive("/products") ? "text-blue-600" : ""}`}
          >
            <span>🛏️</span> პროდუქტები
          </Link>
          <Link
            to="/about"
            className={`flex flex-col items-center gap-1 ${isActive("/about") ? "text-blue-600" : ""}`}
          >
            <span>ℹ️</span> ჩვენზე
          </Link>
          <Link
            to="/contact"
            className={`flex flex-col items-center gap-1 ${isActive("/contact") ? "text-blue-600" : ""}`}
          >
            <span>📞</span> კონტაქტი
          </Link>
        </nav>
